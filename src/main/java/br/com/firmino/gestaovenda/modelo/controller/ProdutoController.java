...
import br.com.firmino.gestaovenda.modelo.util.ProdutoTableModel;
...
public class ProdutoController implements ActionListener, MouseListener, KeyListener {
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        switch(accao) {
            ...
            case "editar" -> editar();
            case "apagar" -> apagar();
            case "adicionarcategoria" -> mostrarTelaCategoria();
            case "salvar" -> salvar();
    ...
    private void validacaoDaCategoria() {
        if(this.dashboard.getComboBoxProdutoCategoria().getSelectedIndex() == 0) {
            String mensagem = "Você deve preencher a categoria";
            mensagemNaTela(mensagem, Color.RED);
            throw new NegocioException(mensagem);
        }
    }
    ...
    private void apagar() {
        Usuario usuario = usuarioLogado();
        if(autenticacaoDao.temPermissao(usuario)){
            System.out.println("Valor de this.produto: " + this.produto);
            if(this.produto != null) {
